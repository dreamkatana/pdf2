...
class SetupDialog(wx.Dialog):
    ...
    def __init__(self, parent, id, title):
        super(SetupDialog, self).__init__(parent, id, title, size=(250, 135))
...
class CustomDialog(wx.Dialog):
    ...
    def __init__(
        ...
        super(CustomDialog, self).__init__(parent, ID, title, pos, size, style)
...
class MyEvtHandler(ogl.ShapeEvtHandler):
    ...
    def OnEndDragLeft(self, x, y, keys=0, attachment=0):
        shape = self.GetShape()
        super(MyEvtHandler, self).OnEndDragLeft(x, y, keys, attachment)
    ...
    def OnSizingEndDragLeft(self, pt, x, y, keys, attch):
        ...
        super(MyEvtHandler, self).OnSizingEndDragLeft(pt, x, y, keys, attch)      
    ...
    def OnMovePost(self, dc, x, y, oldX, oldY, display):
        shape = self.GetShape()
        super(MyEvtHandler, self).OnMovePost(dc, x, y, oldX, oldY, display)
        self.callback()
...
class AppFrame(wx.Frame):
    ...
    def do_about(self, evt):
        ...
        info.WebSite = (
            "https://github.com/reingart/pyfpdf/blob/master/docs/Templates.md",
...
class AppFrame(wx.Frame):
    def __init__(self, paper_size):
        wx.Frame.__init__(self, None, -1, "Visual Template designer for PyFPDF (using wxPython OGL library)", size=(640, 480))
        self.paper_size = paper_size
        self.canvas = ogl.ShapeCanvas(self)
        self.sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.sizer.Add(self.canvas, 1, wx.GROW)
        self.model = ogl.Diagram()
        self.canvas.SetDiagram(self.model)
    ...
    def SetupMenu(self):
        menuBar = wx.MenuBar()
        menu = wx.Menu()
        menu.Append(wx.ID_EXIT, "E&xit", "Exit")
        self.Bind(wx.EVT_MENU, self.OnClose, id=wx.ID_EXIT)
        menuBar.Append(menu, "&File")
        self.SetMenuBar(menuBar)

    def OnClose(self, event):
        self.canvas.DeleteAllShapes()
        self.canvas.Refresh(False)
        self.Destroy()

class MyApp(wx.App):
    def OnInit(self):
        self.paper_size = DEFAULT_PAPER_SIZE  # Initialize paper size here
        frame = AppFrame(self.paper_size)
        frame.Show(True)
        self.SetTopWindow(frame)
        dlg = SetupDialog(frame, -1, "Setup Options")
        dlg.ShowModal()
        self.paper_size = dlg.cmb_paper_size.GetValue()  # Update paper size from dialog
        dlg.Destroy()
        data = CustomDialog.do_input(frame, "Custom Input", ["Field 1", "Field 2"], {"Field 1": 123, "Field 2": "abc"})
        print("Data:", data)
        return True
